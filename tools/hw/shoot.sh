...
CORE=$(ssh -o BatchMode=yes root@$MISTER 'cat /tmp/CORENAME 2>/dev/null')
curl -s -m 20 -X POST "http://$MISTER:8182/api/screenshots" >/dev/null
sleep 3
NEW=$(ssh -o BatchMode=yes root@$MISTER "ls -t /media/fat/screenshots/$CORE/*.png 2>/dev/null | head -1")
[ -z "$NEW" ] && { echo "$LABEL: NO SCREENSHOT (core=$CORE)"; exit 2; }
ssh -o BatchMode=yes root@$MISTER "cat '$NEW'" > "$OUT/$LABEL.png" || exit 3

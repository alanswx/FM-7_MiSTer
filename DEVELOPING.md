# FM-7 for MiSTer — developer notes

*User documentation is in [Readme.md](Readme.md). This file is the working
reference: how the core is built, tested and verified.*
...
**Multi-disk containers work.** 51 images in the Neo Kobe set hold two to six
complete disks in one `.d77`/`.d88`; the `Disk 1 image` / `Disk 2 image` OSD
rows pick which one a drive presents. Verified on hardware: `XANADU.D77` gives
three different programs from one file, and index 0 is byte-identical to the
standalone dump. Sub-disks past the 1 MB mark are not reachable yet — the
container walk is 20-bit — and an out-of-range index clamps to the last
reachable disk.

**The Spanish Secoinsa FM-7 is supported** through a replaceable system ROM
set — see [docs/ROMSETS.md](docs/ROMSETS.md).

Not supported: 2DD media.
...
| [docs/ROMSETS.md](docs/ROMSETS.md) | replaceable system ROM sets, and the Secoinsa FM-7 |
...
**GPLv3** — `LICENSE` is the GPLv3 text. The core's own files carry the MiSTer
framework's "version 2 ... or (at your option) any later version" grant, and
`rtl/jt12/` is jotego's jt12/jt49 under GPL**v3**-or-later; the combination is
permitted by that grant, so the combined work ships as GPLv3. `jt03` is the
FM77AV's YM2203 and also supplies the FM-7's PSG, so removing it is not an
option that leaves a working core.

`LICENSE-GPLv2` is retained because the per-file notices in the core's own
sources refer to it; it is not the licence of the combined work. ROM images in
`rtl/roms/` are not GPL — see [LICENSE-NOTICE.md](LICENSE-NOTICE.md).
